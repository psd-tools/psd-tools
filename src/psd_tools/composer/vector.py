...
def draw_stroke(backdrop, layer, vector_mask=None):
    ...
    # Draw mask.
    stroke_width = float(setting.get("strokeStyleLineWidth", 1.0))
...
def draw_pattern_fill(size, psd, setting):
    ...
    scale = float(setting.get(Key.Scale, 100.0)) / 100.0
    if scale != 1.0:
        panel = panel.resize(
            (
...
def draw_gradient_fill(size, setting):
    ...
    scale = float(setting.get(Key.Scale, 100.0)) / 100.0
    ratio = angle % 90
    scale *= (90.0 - ratio) / 90.0 * size[0] + (ratio / 90.0) * size[1]
    X, Y = np.meshgrid(
...
def _apply_color_map(grad, Z):
    ...
    if gradient_form == Enum.ColorNoise:
        ...
        G = rng.binomial(1, 0.5, (256, len(maximum))).astype(np.float)
        ...
        for stop in grad.get(Key.Colors, []):
            ...
            location = int(stop.get(Key.Location)) / 4096.0
