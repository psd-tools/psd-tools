...
from tests.utils import full_name, FuzzyInt, with_psb
...
PIXEL_COLORS = with_psb((
    ...
    # XXX: what is this test about?
    ('smart-object-slice.psd', (70, 80), (0xAC, 0x19, 0x19)),
))

TRANSPARENCY_PIXEL_COLORS = (
    ('transparentbg-gimp.psd', (14, 14), (0xFF, 0xFF, 0xFF, 0x13)),
    # why gimp shows it as F2F4C2 ?
    ('2layers.psd', (70, 30), (0xF1, 0xF3, 0xC1)),
...
MASK_PIXEL_COLORS = with_psb((
    # this is a clipped point
    ('clipping-mask.psd', (182, 68), (0xDA, 0xE6, 0xF7)),
    # mask truncates the layer here
    ('mask.psd', (87, 7), (0xFF, 0xFF, 0xFF)),
))
...
@pytest.mark.parametrize(["get_color"], BACKENDS)
@pytest.mark.parametrize(
    ["filename", "layer_num", "point", "color"],
    LAYER_COLORS
)
...
@pytest.mark.parametrize(
    ["filename", "point", "color"],
    PIXEL_COLORS + MASK_PIXEL_COLORS + TRANSPARENCY_PIXEL_COLORS
)
