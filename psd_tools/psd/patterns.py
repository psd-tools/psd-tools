...
class Patterns(ListElement):
    ...
    def write(self, fp, **kwargs):
        written = 0
        for item in self:
            written += write_length_block(fp, lambda f: item.write(f), padding=4)
        return written
...
class Pattern(BaseElement):
    ...
    image_mode = attr.ib(default=ColorMode, converter=ColorMode, validator=in_(ColorMode))
...
class VirtualMemoryArray(BaseElement):
    ...
    def get_data(self, version=1):
        ...
        return decompress(self.data, self.compression, width, height, self.depth, version)

    def set_data(self, size, data, depth, compression=0, version=1):
        self.data = compress(data, compression, size[0], size[1], depth, version)
